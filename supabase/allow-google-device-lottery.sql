...
begin
  ...
  if not v_has_google_email and v_participant_hash is null then
  ...
  where lottery_draws.merchant_id = v_card.merchant_id
    and (
      ...
      or (v_participant_hash is null and v_has_google_email and lottery_draws.user_id = v_user)
    )
...
declare
  ...
  v_participant_token text := nullif(trim(coalesce(p_participant_token, '')), '');
  v_participant_hash text := null;
  v_task_link_id text := nullif(trim(coalesce(p_task_link_id, '')), '');
  v_uses_google_device_lottery boolean := false;
  v_has_google_email boolean := false;
  v_card public.tap_cards%rowtype;
  v_draw public.lottery_draws%rowtype;
...
begin
  ...
  select exists(
    ...
    where action_links.id = v_task_link_id
      and action_links.card_id = v_card.id
      and action_links.enabled = true
      and action_links.id = 'google'
  ) into v_uses_google_device_lottery;
  ...
  if not v_has_google_email and not (v_uses_google_device_lottery and v_participant_hash is not null) then
    raise exception 'google_email_required';
  end if;
  ...
  where lottery_draws.merchant_id = v_card.merchant_id
    and (
      (v_participant_hash is not null and v_uses_google_device_lottery and lottery_draws.participant_token_hash = v_participant_hash)
      or (v_has_google_email and lottery_draws.user_id = v_user)
    )
  order by created_at desc
  limit 1;
  ...
  insert into public.lottery_draws (
    ...
    v_card.store_id,
    v_card.id,
    case when v_has_google_email then v_user else null end,
    case when v_uses_google_device_lottery then v_participant_hash else null end,
    v_task_link_id,
    '抽奖处理中',
    '',
    'pending'
